...
const Palette = ({board_manager}) => {
  ...
  const handleChange = (color,event) => {
    let rgba = color.rgb;
    setColor("rgba(" + rgba["r"] + "," + rgba["g"] + "," + rgba["b"] + "," + rgba["a"] + ")");
  }
  ...
  return (
    <Flex wrap="wrap" w={{ base: "100%", sm: "70px", md: "100px", lg: "360px" }}
      onContextMenu={(e)=>{e.preventDefault(); console.log("onContextMenu");}}
    >
      {COLOR_PICKER_TYPE == "default" && colors.map((color, i) => (
        <Flex
      ...
      {COLOR_PICKER_TYPE == "chrome" && (
        <ChromePicker color={curColor} onChange={handleChange} onChangeComplete={ handleChangeComplete }/>
      )}
    </Flex>
  );
};
